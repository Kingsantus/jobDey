...
const JobPosted = () => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            alert(data.message); // Use the parsed message for alert
    ...
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
        ...
        <section className="py-1 bg-blueGray-50">
            <div className="w-full xl:w-10/12 mb-12 xl:mb-0 px-4 mx-auto mt-5">
                    ...
                    <div className="rounded-t mb-0 px-4 py-3 border-0">
                        <div className="flex flex-wrap items-center">
                            ...
                            <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                                <Link to="/post-job">
                                    <button className="bg-blue text-white active:bg-blue/45 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post A New Job</button>
                                </Link>
                            </div>
                        </div>
                    </div>
                            ...
                            <table className="items-center bg-transparent w-full border-collapse">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Approved</th>
                                ...
                                <tbody>
                                    {currentJobs.map((job, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                <button className='bg-red-600 py-2 px-6 text-white rounded-sm' onClick={() => handleDelete(job._id)}>Delete</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                {/* Pagination */}
                <div className='flex justify-center text-base space-x-8 mb-4'>
                    {
                        currentPage > 1 && (
